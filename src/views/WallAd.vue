    ...
    <div>
        ...
        <div class="form">
            <div class="inner_form">
                ...
                <div class="section">
                    <label for="attachments">Введите аттрибуты элементов, которые хотите видеть</label>
                    <input id="attachments" type="text" v-model="attachments">
                </div>
                <div class="section">
                    <label for="message">Введите текст, который будет приложен к посту</label>
...
export default({
    ...
    methods: {
        createCampaign() {
            axios.get('api/ads.createCampaigns?account_id=' + auth_data.cab_id + '&data=[{"client_id":"' + auth_data.owner_id + '","type":"promoted_posts","name":"camp 9 from ui","status":0}]&access_token=' + auth_data.access_token + '&v=5.131')
                .then((data) => {
        ...
        createAds() {
            ...
            axios.get(url).then((data) => {this.ad_id = data.data.response[0].id})
        },
        create() {
