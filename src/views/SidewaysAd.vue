...
export default {
  ...
  created() {
    axios
      .get(
        "api/ads.getUploadURL?ad_format=1&access_token=" +
  ...
  methods: {
    uploadFile() {
      ...
      axios
        .post("upload-image" + this.imageUrl, formData, {
          headers: {
            "Content-Type": "multipart/form-data",
          },
        })
        .then((data) => {
          this.adsData.photo = data.data.photo;
        });
    },
    createCampaign() {
      axios
        .get(
          "api/ads.createCampaigns?account_id=" +
            auth_data.cab_id +
            '&data=[{"client_id":"344701865","type":"normal","name":"camp from ui","status":0}]&access_token=' +
            auth_data.access_token +
            "&v=5.131"
        )
        .then((data) => {
